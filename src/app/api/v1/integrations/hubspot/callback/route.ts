  ...
  try {
    ...
    const tokenResponse = await fetch(HUBSPOT_TOKEN_URL, {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXT_PUBLIC_URL}/api/v1/integrations/hubspot/callback`,
        code,
      }),
    });
    ...
    const { error } = await supabase.from("integrations").upsert({
      ...
      config: { portal_id: tokenData.hub_id?.toString() },
      is_active: true,
      last_sync_at: new Date().toISOString(),
    }, { onConflict: "user_id,provider" });

    if (error) return NextResponse.json({ error: "Failed to save integration", details: error.message }, { status: 500 });
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_URL}/integrations?connected=hubspot`);
  } catch (error: any) {
