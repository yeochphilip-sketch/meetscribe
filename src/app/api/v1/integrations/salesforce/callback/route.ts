  ...
  try {
    ...
    const tokenResponse = await fetch(SALESFORCE_TOKEN_URL, {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXT_PUBLIC_URL}/api/v1/integrations/salesforce/callback`,
        code,
      }),
    });
    ...
    const { error } = await supabase.from("integrations").upsert({
      ...
      config: { instance_url: tokenData.instance_url, id: tokenData.id },
      is_active: true,
      last_sync_at: new Date().toISOString(),
    }, { onConflict: "user_id,provider" });

    if (error) return NextResponse.json({ error: "Failed to save integration", details: error.message }, { status: 500 });
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_URL}/integrations?connected=salesforce`);
  } catch (error: any) {
